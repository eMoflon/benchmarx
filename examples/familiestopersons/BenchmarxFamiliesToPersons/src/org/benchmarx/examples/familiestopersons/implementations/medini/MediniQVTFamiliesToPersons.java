...
public class MediniQVTFamiliesToPersons extends BXToolForEMF<FamilyRegister, PersonRegister, Decisions> {
	...
	public MediniQVTFamiliesToPersons() {
		...
		// Tell the QVT engine a directory to work in - e.g. to store the trace
		// (meta)models
	...
	/**
	 * Initiates a synchronization between a source and a target model. The medini
	 * QVT engine is initialized, the required metamodels are registered and empty
	 * source and target models are created. Finally a FamilyRegister is added to
	 * the source model and an initial forward transformation is issued to create a
	 * corresponding PersonRegister.
	 */
	@Override
	public void initiateSynchronisationDialogue() {
		// delete content of traces folder
		File tracesFolder = new File(
				"./src/org/benchmarx/examples/familiestopersons/implementations/medini/base/traces");
	...
	/**
	 * Provide the information about the metamodels, which are involved in the
	 * transformation
	 * 
	...
	/**
	 * Before transforming, the engine has to know the model to perform the
	 * transformation on, as well as a directory for the traces to store.
	 * 
	 * @param modelResources   models for the execution - take care of the right
	 *                         order!
	 ...
	 * @param direction      name of the target - must conform to your QVT
	...
	/**
	 * Perform an edit operation on the target model, without propagating the change
	 * to the source model
	 * 
	...
	/**
	 * Perform an edit operation on the source model, without propagating the change
	 * to the target model
	 * 
	 * @param edit : the edit delta
	 */
