...
public class DaughterToFemale_fm_father_0_incoming_SOURCE__FILTER_NAC_SRC_44 extends GenericStatelessSearchActor{
			...
			case "Family_object_SP11": 
				{
					{
						// FILTER_NAC_NODE
						var match_0 = new StatelessDeltaMatch(msg, "DaughterToFemale_fm_father_0_incoming_SOURCE__FILTER_NAC_SRC_44", numberOfNodes, 0, UsingDeltaMode.CREATE);
			...
			case "FamilyMember_object_SP8": 
				{
					if(!lazy_initialization) {
						{
							// fm
							var match_1 = new StatelessDeltaMatch(msg, "DaughterToFemale_fm_father_0_incoming_SOURCE__FILTER_NAC_SRC_44", numberOfNodes, 0, UsingDeltaMode.CREATE);
			...
			case "Family_object_SP11": 
				{
					{
						// FILTER_NAC_NODE
						var match_0 = new StatelessDeltaMatch(msg, "DaughterToFemale_fm_father_0_incoming_SOURCE__FILTER_NAC_SRC_44", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_0.getNodes()[0] = objs[0];
						if(options.trackMatchingProcess)
			...
			case "FamilyMember_object_SP8": 
				{
					{
						// fm
						var match_1 = new StatelessDeltaMatch(msg, "DaughterToFemale_fm_father_0_incoming_SOURCE__FILTER_NAC_SRC_44", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_1.getNodes()[1] = objs[0];
						match_1.registerSignatureIndex(1);
						if(options.trackMatchingProcess)
			...
			{
				{
					...
					var match = new StatelessDeltaMatch(msg, "DaughterToFemale_fm_father_0_incoming_SOURCE__FILTER_NAC_SRC_44", numberOfNodes, 0, UsingDeltaMode.CREATE);
					Object[] objs = match.getNodes();
					objs[0] = msg.source;
					objs[1] = msg.target;
					if(options.trackMatchingProcess)
	...
	protected void removeReference(ReferenceDeleted msg) {
			...
			case "FamiliesSmartEMF.Family_father_FamilyMember": 
		{
			...
			var match = new StatelessDeltaMatch(msg, "DaughterToFemale_fm_father_0_incoming_SOURCE__FILTER_NAC_SRC_44", numberOfNodes, 0, UsingDeltaMode.DELETE);
			Object[] objs = match.getNodes();
			objs[0] = msg.source;
			objs[1] = msg.target;
			if(options.trackMatchingProcess)
		...
		switch(message.nodeName) {
			case "DaughterToFemale__FWD_47": {
				switch(message.queryComponentId) {
					case 2: {
						var deltaMatch = new StatelessDeltaMatch(message, "DaughterToFemale_fm_father_0_incoming_SOURCE__FILTER_NAC_SRC_44", 2, 0, message.usedDelta);
