...
//						start(edge_explorer_1_orchestration, StatelessInputType.OBJECT, match_1);
			...
			case "Female_object_SP1": 
				{
					{
						// p
						var match_2 = new StatelessDeltaMatch(msg, "DaughterToFemale__BWD_70", numberOfNodes, 0, UsingDeltaMode.CREATE);
			...
			case "FamilyRegister_object_SP6": 
				{
					{
						// families
						var match_0 = new StatelessDeltaMatch(msg, "DaughterToFemale__BWD_70", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_0.getNodes()[0] = objs[0];
						match_0.registerSignatureIndex(0);
						if(options.trackMatchingProcess)
			...
			case "PersonRegister_object_SP7": 
				{
					{
						// persons
						var match_1 = new StatelessDeltaMatch(msg, "DaughterToFemale__BWD_70", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_1.getNodes()[1] = objs[0];
						match_1.registerSignatureIndex(1);
						if(options.trackMatchingProcess)
			...
			case "Female_object_SP1": 
				{
					{
						// p
						var match_2 = new StatelessDeltaMatch(msg, "DaughterToFemale__BWD_70", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_2.getNodes()[2] = objs[0];
						match_2.registerSignatureIndex(2);
						if(options.trackMatchingProcess)
		...
		case "PersonsSmartEMF.PersonRegister_persons_Person": 
			{
				if(msg.target instanceof PersonsSmartEMF.Female) 
				{
					...
					var match = new StatelessDeltaMatch(msg, "DaughterToFemale__BWD_70", numberOfNodes, 0, UsingDeltaMode.CREATE);
					Object[] objs = match.getNodes();
					objs[1] = msg.source;
					objs[2] = msg.target;
					if(options.trackMatchingProcess)
	...
	protected void removeReference(ReferenceDeleted msg) {
		...
		if(msg.target instanceof PersonsSmartEMF.Female) 
		{
			...
			var match = new StatelessDeltaMatch(msg, "DaughterToFemale__BWD_70", numberOfNodes, 0, UsingDeltaMode.DELETE);
			Object[] objs = match.getNodes();
			objs[1] = msg.source;
			objs[2] = msg.target;
			if(options.trackMatchingProcess)
