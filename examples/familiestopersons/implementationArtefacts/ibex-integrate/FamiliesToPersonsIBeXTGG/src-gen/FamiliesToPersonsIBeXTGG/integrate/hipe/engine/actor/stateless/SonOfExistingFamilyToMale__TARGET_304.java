...
public class SonOfExistingFamilyToMale__TARGET_304 extends GenericStatelessSearchActor{
			...
			case "PersonRegister_object_SP1": 
				{
					if(!lazy_initialization) {
						{
							// persons
							var match_0 = new StatelessDeltaMatch(msg, "SonOfExistingFamilyToMale__TARGET_304", numberOfNodes, 0, UsingDeltaMode.CREATE);
			...
			case "Male_object_SP0": 
				{
					{
						// p
						var match_1 = new StatelessDeltaMatch(msg, "SonOfExistingFamilyToMale__TARGET_304", numberOfNodes, 0, UsingDeltaMode.CREATE);
			...
			case "PersonRegister_object_SP1": 
				{
					{
						// persons
						var match_0 = new StatelessDeltaMatch(msg, "SonOfExistingFamilyToMale__TARGET_304", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_0.getNodes()[0] = objs[0];
						match_0.registerSignatureIndex(0);
						if(options.trackMatchingProcess)
			...
			case "Male_object_SP0": 
				{
					{
						// p
						var match_1 = new StatelessDeltaMatch(msg, "SonOfExistingFamilyToMale__TARGET_304", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_1.getNodes()[1] = objs[0];
						match_1.registerSignatureIndex(1);
						if(options.trackMatchingProcess)
		...
		case "PersonsSmartEMF.PersonRegister_persons_Person": 
			{
				if(msg.target instanceof PersonsSmartEMF.Male) 
				{
					...
					var match = new StatelessDeltaMatch(msg, "SonOfExistingFamilyToMale__TARGET_304", numberOfNodes, 0, UsingDeltaMode.CREATE);
					Object[] objs = match.getNodes();
					objs[0] = msg.source;
					objs[1] = msg.target;
					if(options.trackMatchingProcess)
	...
	protected void removeReference(ReferenceDeleted msg) {
		...
		if(msg.target instanceof PersonsSmartEMF.Male) 
		{
			...
			var match = new StatelessDeltaMatch(msg, "SonOfExistingFamilyToMale__TARGET_304", numberOfNodes, 0, UsingDeltaMode.DELETE);
			Object[] objs = match.getNodes();
			objs[0] = msg.source;
			objs[1] = msg.target;
			if(options.trackMatchingProcess)
