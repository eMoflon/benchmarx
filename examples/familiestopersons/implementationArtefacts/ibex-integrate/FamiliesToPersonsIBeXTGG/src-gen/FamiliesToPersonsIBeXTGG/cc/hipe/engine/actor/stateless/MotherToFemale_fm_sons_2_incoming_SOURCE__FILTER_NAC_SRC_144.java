...
public class MotherToFemale_fm_sons_2_incoming_SOURCE__FILTER_NAC_SRC_144 extends GenericStatelessSearchActor{
			...
			case "Family_object_SP7": 
				{
					if(!lazy_initialization) {
						{
							// FILTER_NAC_NODE
							var match_0 = new StatelessDeltaMatch(msg, "MotherToFemale_fm_sons_2_incoming_SOURCE__FILTER_NAC_SRC_144", numberOfNodes, 0, UsingDeltaMode.CREATE);
			...
			case "FamilyMember_object_SP7": 
				{
					{
						// fm
						var match_1 = new StatelessDeltaMatch(msg, "MotherToFemale_fm_sons_2_incoming_SOURCE__FILTER_NAC_SRC_144", numberOfNodes, 0, UsingDeltaMode.CREATE);
			...
			case "Family_object_SP7": 
				{
					{
						// FILTER_NAC_NODE
						var match_0 = new StatelessDeltaMatch(msg, "MotherToFemale_fm_sons_2_incoming_SOURCE__FILTER_NAC_SRC_144", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_0.getNodes()[0] = objs[0];
						if(options.trackMatchingProcess)
			...
			case "FamilyMember_object_SP7": 
				{
					{
						// fm
						var match_1 = new StatelessDeltaMatch(msg, "MotherToFemale_fm_sons_2_incoming_SOURCE__FILTER_NAC_SRC_144", numberOfNodes, 0, UsingDeltaMode.DELETE);
						match_1.getNodes()[1] = objs[0];
						match_1.registerSignatureIndex(1);
						if(options.trackMatchingProcess)
			...
			{
				{
					...
					var match = new StatelessDeltaMatch(msg, "MotherToFemale_fm_sons_2_incoming_SOURCE__FILTER_NAC_SRC_144", numberOfNodes, 0, UsingDeltaMode.CREATE);
					Object[] objs = match.getNodes();
					objs[0] = msg.source;
					objs[1] = msg.target;
					if(options.trackMatchingProcess)
	...
	protected void removeReference(ReferenceDeleted msg) {
			...
			case "FamiliesSmartEMF.Family_sons_FamilyMember": 
		{
			...
			var match = new StatelessDeltaMatch(msg, "MotherToFemale_fm_sons_2_incoming_SOURCE__FILTER_NAC_SRC_144", numberOfNodes, 0, UsingDeltaMode.DELETE);
			Object[] objs = match.getNodes();
			objs[0] = msg.source;
			objs[1] = msg.target;
			if(options.trackMatchingProcess)
	...
	protected void processMatchRequest(MatchRequest message) {
		...
		switch(message.nodeName) {
			case "MotherToFemale__CONSISTENCY_157": {
				switch(message.queryComponentId) {
					case 1: {
						var deltaMatch = new StatelessDeltaMatch(message, "MotherToFemale_fm_sons_2_incoming_SOURCE__FILTER_NAC_SRC_144", 2, 0, message.usedDelta);
