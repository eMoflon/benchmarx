...
public class NMFFamiliesToPersonsIncremental implements BXTool<FamilyRegister, PersonRegister, Decisions> {
	private static final String NMF_DLL = "../implementationArtefacts/NMF/bin/NMFSolution.exe";
	...
	public void performAndPropagateTargetEdit(Supplier<IEdit<PersonRegister>> edit) {
		...
		recorder.observePersonsRegister(trg, false);
	    recorder.WriteEdit(edit.get());		
	...
	@SuppressWarnings("deprecation")
	private long propagateConflict(ChangeRecorder recorder1, ChangeRecorder recorder2) {
		String changes1 = recorder1.stopAndExport();
		String changes2 = recorder2.stopAndExport();
		try {
			File tempFile1 = File.createTempFile("change", ".xmi");
			Files.write(changes1, tempFile1, Charsets.UTF_8);
			File tempFile2 = File.createTempFile("change", ".xmi");
			Files.write(changes2, tempFile2, Charsets.UTF_8);
			writer.write("PropagateConflicted ");
			writer.write(tempFile1.getAbsolutePath());
			writer.write(" ");
			writer.write(tempFile2.getAbsolutePath());
			writer.write("\n");
			writer.flush();
			String result = reader.readLine();
			tempFile1.delete();
	...
	public void performAndPropagateSourceEdit(Supplier<IEdit<FamilyRegister>> edit) {
		...
		setUpdatePolicy();
		ChangeRecorder recorder = new ChangeRecorder();
		recorder.observeFamilyRegister(src, false);
		recorder.WriteEdit(edit.get());
		long actual = propagate(recorder);
	...
	private void runNMF() {
		try {
			File pathToExecutable = new File(NMF_DLL);
	...
	public void performAndPropagateEdit(Supplier<IEdit<FamilyRegister>> sourceEdit,
		...
		long start = System.nanoTime();
		setUpdatePolicy();
		ChangeRecorder personsRecorder = new ChangeRecorder();
		personsRecorder.observePersonsRegister(trg, false);
		personsRecorder.WriteEdit(targetEdit.get());
		ChangeRecorder familiesRecorder = new ChangeRecorder();
		familiesRecorder.observeFamilyRegister(src, false);
		familiesRecorder.WriteEdit(sourceEdit.get());
		
		long actual = propagateConflict(familiesRecorder, personsRecorder);
		src = readModel("SaveFamilies");
		trg = readModel("SavePersons");
		long end = System.nanoTime();
