...
namespace TTC2026.FamiliesToPersons.NMF.Families
{
    ...
    /// </summary>
    public class FamilySonsCollection : ObservableOppositeOrderedSet<IFamily, IFamilyMember>
    {
        ...
        protected override void SetOpposite(IFamilyMember item, IFamily newParent)
        {
            if ((newParent != null))
            {
                item.ParentChanged += this.OnItemParentChanged;
                item.SonsInverse = newParent;
            }
            else
            {
                item.ParentChanged -= this.OnItemParentChanged;
                if ((item.SonsInverse == this.Parent))
                {
                    item.SonsInverse = newParent;
                }
            }
