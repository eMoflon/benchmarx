...
namespace TTC2026.FamiliesToPersons.NMF
{
    public class FamiliesToPersonsSynchronization : ReflectiveSynchronization
    {
        ...
        public class MemberToMale : SynchronizationRule<IFamilyMember, IMale>
        {
            ...
            public override bool ShouldCorrespond(IFamilyMember left, IMale right, ISynchronizationContext context)
            {
                return left.GetFullName() == right.Name;
            }
        }
        ...
        internal class FamilyMemberCollection : CustomCollection<IFamilyMember>
        {
            ...
            {
                var family = item.Parent as IFamily;
                item.Delete();
                //if (family != null && !family.Children.Any())
                //{
                //    family.Delete();
                //}
                return true;
    ...
    public static class Helpers
    {
        ...
        {
            var isInTransaction = ExecutionEngine.Current.TransactionActive;
            if (!isInTransaction)
            {
